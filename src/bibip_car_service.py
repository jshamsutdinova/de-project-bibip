...
class CarService:
    ...
    def add_model(self, model: Model) -> Model:
        ...
        table = TableUtils(self._get_data_file('models'))
        index = IndexUtils(self._get_index_file('models'))

        row = model.serialize()
        line_number = table.append_row(row)
        index.add(line_number, model.index())

    # Задание 1. Сохранение автомобилей и моделей
    def add_car(self, car: Car) -> Car:
        """Add new car to the database."""
        table = TableUtils(self._get_data_file('cars'))
        index = IndexUtils(self._get_index_file('cars'))

        row = car.serialize()
        line_number = table.append_row(row)
        index.add(line_number, car.index())

    # Задание 2. Сохранение продаж.
    def sell_car(self, sale: Sale) -> Car:
        """Save car sale to the database."""
        sales_table = TableUtils(self._get_data_file('sales'))
        sales_index = IndexUtils(self._get_index_file('sales'))

        row = sale.serialize()
        line_number = sales_table.append_row(row)
        sales_index.add(line_number, sale.index())

        # Find car index
        car_table = TableUtils(self._get_data_file('cars'))
        car_index = IndexUtils(self._get_index_file('cars'))

        # Find record in the Car table
        car_line_number = car_index.get(sale.car_vin)
        row = car_table.get_row(car_line_number)
        car = Car.parse_row(row)

        # Change status of car
        car.status = CarStatus.sold
        new_row = car.serialize()
        car_table.write_row(car_line_number, new_row)
    ...
    def get_car_info(self, vin: str) -> CarFullInfo | None:
        ...
        model_index = IndexUtils(self._get_index_file('models'))
        model_table = TableUtils(self._get_data_file('models'))

        # Find a record in Car table
        car_line_num = car_index.get(vin)

        if car_line_num is None:
            return None

        row = car_table.get_row(car_line_num)
        car = Car.parse_row(row)

        # Find a record in Model table
        model_line_num = None
        model_line_num = model_index.get(car.model)

        if model_line_num is None:
            return None

        row = model_table.get_row(model_line_num)
    ...
    def update_vin(self, vin: str, new_vin: str) -> Car:
        ...
        # Get car
        car_line_num = car_index.get(vin)
        car_row = car_table.get_row(car_line_num)
        car = Car.parse_row(car_row)

        # Update car
        car.vin = new_vin
        new_row = car.serialize()
        car_table.write_row(car_line_num, new_row)
        car_index.update(vin, new_vin)
    ...
    def revert_sale(self, sales_number: str) -> Car:
        ...
        sale_row = sales_table.get_row(sale_line_number)
        ...
        if not sale.is_deleted:
            ...
            sales_table.write_row(sale_line_number, new_row)

            car_index = IndexUtils(self._get_index_file('cars'))
            car_table = TableUtils(self._get_data_file('cars'))

            car_line_number = car_index.get(sale.car_vin)
            car_row = car_table.get_row(car_line_number)
            car = Car.parse_row(car_row)

            car.status = CarStatus.available
            new_row_car = car.serialize()
            car_table.write_row(car_line_number, new_row_car)
